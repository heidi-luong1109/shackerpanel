var soft = {
    ...
    get_list: function(page, type, search) {
        ...
        bt.soft.get_soft_list(page, type, search, function(rdata) {
            ...
            // if (type == 10) {
            //     $("#updata_pro_info").html('<div class="alert alert-danger" style="margin-bottom:15px"><strong>' + lan.soft.bt_developer + '</strong><a class="btn btn-success btn-xs va0" href="https://www.shackerPanel.com" title="' + lan.soft.get_third_party_apps + '" style="margin-left: 8px" target="_blank">' + lan.soft.get_third_party_apps + '</a><input type="file" style="display:none;" accept=".zip,.tar.gz" id="update_zip" multiple="multiple"><button class="btn btn-success btn-xs" onclick="soft.update_zip_open()" style="margin-left:8px">' + lan.soft.import_plug + '</button></div>')
            // } else if (type == 11) {
    ...
    set_soft_tips:function(el,type){
        ...
        if(type == 10){
            explain.text('Security Reminder: shackerPanel officially conducted a security audit before the third-party plug-in was put on the shelves, but there may be security risks. Please check it out before using it in the production environment.');
        ...
        $.post('/deployment?action=GetList', pdata, function(rdata) {
            ...
            if ($(".onekey-type").attr("class") === undefined) {
                ...
                tbody = '<div class="alert alert-info" style="margin-bottom: 10px;">\
                        <strong class="mr5">shackerPanel one-click deployment has been launched, and we invite global outstanding projects to settle in (limited to project officials)</strong>\
            ...
            for (var i = 0; i < rdata.list.length; i++) {
                ...
                zbody += '<tr>' +
                    ...
                    '<td><a class="btlink" target="_blank" href="' + rdata.list[i].official + '">' + (rdata.list[i].author == 'shackerPanel' ? rdata.list[i].title : rdata.list[i].author) + '</a></td>' +
...
function onekeyCodeSite(codename, versions, title, enable_functions) {
    $.post('/site?action=GetPHPVersion', function(rdata) {
        ...
        add = layer.open({
            type: 1,
            title: "shackerPanel One-Click [" + title + ']',
            area: '560px',
            closeBtn: 2,
            shadeClose: false,
            content: con
